...
public class MainClass {
	...
	public static void main(String[] args) {
		...
		//COMPROBADOR DE QUE SE HAYA INTRODUCIDO EL NUMERO CORRECTO
		do {
			if (sc.hasNextInt())  {
				option = sc.nextInt();
		...
		while (flg01) {
			...
			System.out.println("Introduce el nº de DNI de tu personaje");
			...
			do {
				if (sc.hasNextInt()) {
					...
					if (numeroDni.length() == 8) {
						System.out.println("OCUPA 8 ESPACIOS");
						correcto = true;
					} else {
						System.out.println("Error, la cantidad de numero introduciods no coincide con los requeridos para el dni");
					}
				} else {
					System.out.println("Error, deben ser números.");
				}
			} while (!correcto);
			System.out.println("Introduce la letra de DNI de tu personaje");
			String letraDni;
			int numero = (int) (numeroDni);
		}
